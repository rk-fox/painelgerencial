...
const ScheduleAdjustment: React.FC = () => {
    ...
    const fetchMembersWithDiarias = async (filterSector?: string) => {
        ...
        let query = supabase
            .from("members")
            .select(
                "id, name, war_name, rank, abrev, status, sector, last_promotion_date, guia_antiguidade",
            );
    ...
    const estimatedDiarias = dataInicio && dataFim
        ? calculateDuration(dataInicio, dataFim)
        : 0;
    ...
    const handleSave = async () => {
        ...
        if (currentUser?.sector === "CH" && !isEditMode && !selectedSector) {
            alert(
                "Por favor, selecione para qual seção (Capacidade ATC ou Espaço Aéreo) esta missão será criada.",
            );
            return;
        }
        ...
        try {
            if (isEditMode) {
                ...
                // If mission was updated, update the related task's dates if it exists
                if (updatedMission && updatedMission.task_id) {
                    ...
                    const detailedDescription =
                        `Missão: ${nome} - Destino: ${local} - Deslocamento: ${deslocamento} - Período: ${
                            formatDateBr(dataInicio)
                        } a ${formatDateBr(dataFim)} - Equipe: ${
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-[#4c739a] dark:text-slate-400">
                        Gerencie a distribuição de diárias e membros da equipe
                        técnica.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="relative">
                                        <span className="material-symbols-outlined absolute left-3 top-3 text-slate-400 text-xl">
                                            location_on
                                        </span>
                                        <input
                                            value={local}
                                        ...
                                        <input
                                            ...
                                            className={`w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 focus:ring-primary h-12 pl-11 text-sm ${
                                                selectedTeam.length > 0
                                                    ? "opacity-70 cursor-not-allowed font-bold text-primary"
                                                    : ""
                                            }`}
                                            placeholder="Qtd de integrantes (se equipe estiver vazia)"
                                        />
                                        {selectedTeam.length > 0 && (
                                            <span className="absolute right-3 top-3 text-[10px] font-bold text-primary bg-primary/10 px-2 py-1 rounded-lg">
                        ...
                        <div className="border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-4">
                                <button
                                    ...
                                    className="size-8 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all active:scale-95"
                                >
                        ...
                        <div className="p-6">
                            ...
                            <div className="grid grid-cols-7 gap-2">
                                {Array.from({
                                    ...
                                    <div
                                        key={`empty-${i}`}
                                        className="h-10 flex items-center justify-center text-slate-300 text-sm"
                                    >
                                    </div>
                                ))}
                                {Array.from({
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm dark:text-slate-200">
                                            ...
                                            <span className="font-bold text-primary">
                                                {formatDateDisplay(dataInicio)}
                                            </span>{" "}
                                            a{" "}
                                            <span className="font-bold text-primary">
                                                {formatDateDisplay(dataFim)}
                                            </span>{" "}
                                            — {estimatedDiarias}{" "}
                                            diárias estimadas
                                        </p>
                ...
                <div className="xl:col-span-4 space-y-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {selectedTeam.map((memberId) => {
                                    ...
                                    return (
                                        <div
                                            key={memberId}
                                            className="flex items-center gap-2 bg-white dark:bg-slate-700 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 shadow-sm"
                                        >
                    ...
                    <div className="bg-primary rounded-2xl p-6 text-white shadow-xl shadow-primary/20 relative overflow-hidden">
                        ...
                        <h3 className="text-xs font-black opacity-70 uppercase tracking-[0.2em] mb-6">
                            Resumo Estimado
                        </h3>
                        <div className="space-y-4 relative z-10">
                            ...
                            <div className="flex justify-between items-center border-b border-white/10 pb-3">
                                ...
                                <span className="font-bold text-lg">
                                    {String(selectedTeam.length).padStart(
                                        2,
                                        "0",
                                    )}
                                </span>
