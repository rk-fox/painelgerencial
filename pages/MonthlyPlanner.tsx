...
const MonthlyPlanner: React.FC = () => {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            ).in("abrev", ABREV_ORDER);
            let allMembersQuery = supabase.from("members").select(
                "id, name, war_name, rank, abrev, avatar, specialty, sector, last_promotion_date, guia_antiguidade, status",
            );
            ...
            if (sector && (sector === "CP" || sector === "EA")) {
                ...
                allMembersQuery = allMembersQuery.eq("sector", sector).in(
                    "abrev",
                    ABREV_ORDER,
                );
            }

            const [missionsRes, unavailRes, membersRes, allMembersRes] =
                await Promise.all([
                    missionsQuery,
                    unavailQuery,
                    membersQuery,
    ...
    const getTasksForDay = (
        ...
        const dayRealTasks = filteredTasks.filter((t) =>
            toLocalDateString(t.start_date) === dateString
        );
        dayRealTasks.forEach((t) =>
            results.push({ task: t, isProjected: false })
        );
        ...
        masterTasks.forEach((task) => {
            ...
            const tStart = new Date(
                task.start_date.split("T")[0] + "T12:00:00",
            );
            ...
            if (p === "diaria") matches = isBusinessDay(date);
                ...
                const diffDays = Math.floor(
                    (compareDate.getTime() - tStart.getTime()) /
                        (1000 * 60 * 60 * 24),
                );
                matches = diffDays >= 0 && diffDays % 14 === 0;
            } else if (p === "mensal") {
                matches =
                    day === getFirstBusinessDayOfMonth(targetYear, targetMonth);
            }

            if (matches) {
                if (
                    realNamesToday.has(task.name) ||
                    projectedNames.has(task.name)
                ) return;
                ...
                const currentStatus = history?.find((t) =>
                    new Date(t.start_date.split("T")[0] + "T12:00:00") <=
                        compareDate
                );
                ...
                if (currentStatus) {
                    ...
                    if (currentStatus.recurrence_active) {
                        results.push({
                            task: {
                                ...task,
                                assigned_to: currentStatus.assigned_to,
                            },
                            isProjected: true,
                        });
        ...
        results.sort((a, b) => {
            const orderA = periodicityOrder[a.task.periodicity.toLowerCase()] ||
                99;
            const orderB = periodicityOrder[b.task.periodicity.toLowerCase()] ||
                99;
    ...
    const saveUnavail = async () => {
        if (
            !unavailForm.member || !unavailForm.type ||
            !unavailForm.start_date || !unavailForm.end_date
        ) {
            console.warn("Missing fields in unavailability form", unavailForm);
            return;
        }
        // Validação: Atividade requer seleção de período
        if (unavailForm.type === "Atividade" && !unavailForm.atividade) {
            alert(
                "Selecione o período (Manhã, Tarde ou Total) para o tipo Atividade.",
            );
            return;
        }
        try {
            ...
            const payload: Record<string, any> = {
                ...
                detalhes:
                    ["Home Office", "Dispensa", "Outros", "Atividade"].includes(
                            unavailForm.type,
                        )
                        ? (unavailForm.detalhes || null)
                        : null,
                atividade: unavailForm.type === "Atividade"
                    ? unavailForm.atividade
                    : null,
            };
    ...
    const deleteUnavail = async () => {
        ...
        try {
            const { error } = await supabase.from("unavailability").delete().eq(
                "id",
                unavailForm.id,
            );
            if (error) throw error;
    ...
    const unavailColor = (type: string) => {
        switch (type) {
            case "SALOP":
                return "bg-red-100 border-red-300 text-red-700 dark:bg-red-900/30 dark:border-red-800 dark:text-red-400";
            case "CAIS":
                return "bg-orange-100 border-orange-300 text-orange-700 dark:bg-orange-900/30 dark:border-orange-800 dark:text-orange-400";
            case "Férias":
                return "bg-emerald-100 border-emerald-300 text-emerald-700 dark:bg-emerald-900/30 dark:border-emerald-800 dark:text-emerald-400";
            case "HO":
                return "bg-sky-100 border-sky-300 text-sky-700 dark:bg-sky-900/30 dark:border-sky-800 dark:text-sky-400";
            case "Dispensa":
                return "bg-yellow-100 border-yellow-300 text-yellow-700 dark:bg-yellow-900/30 dark:border-yellow-800 dark:text-yellow-400";
            case "RISAER":
                return "bg-indigo-100 border-indigo-300 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-800 dark:text-indigo-400";
            case "Aniversário":
                return "bg-pink-100 border-pink-300 text-pink-700 dark:bg-pink-900/30 dark:border-pink-800 dark:text-pink-400";
            case "Atividade":
                return "bg-cyan-100 border-cyan-300 text-cyan-700 dark:bg-cyan-900/30 dark:border-cyan-800 dark:text-cyan-400";
            case "Outros":
    ...
    const renderTaskItem = (
        ...
        day: number,
        isProjected: boolean,
        _tooltipDown: boolean = false,
    ) => {
        const assignedMemberId = task.assigned_to || task.preassigned_to;
        const assignedMember = assignedMemberId
            ? allMembersMap[assignedMemberId]
            : null;

        const periodColors: Record<string, string> = {
            "diaria":
                "bg-slate-100 border-slate-200 text-slate-700 dark:bg-slate-800/40 dark:border-slate-700 dark:text-slate-300",
            "semanal":
        ...
        return (
            <div
                ...
                title={`${task.name} (${task.periodicity})${
                    assignedMember
                        ? ` — Atribuído a: ${assignedMember.rank} ${
                            assignedMember.war_name || assignedMember.name
                        }`
                        : ""
                }`}
                className={`group/task relative flex items-center h-12 ${colors} border rounded-lg px-3 overflow-visible transition-all hover:shadow-md ${
                    isProjected ? "cursor-default" : "cursor-pointer"
                }`}
            >
                <div
                    className={`absolute left-0 top-0 bottom-0 w-1.5 ${barColor}`}
                />
                ...
                <div
                    className={`${
                        assignedMember ? "" : "ml-1"
                    } w-full flex flex-col justify-center gap-0.5 min-w-0`}
                >
    ...
    const renderUnavailItem = (u: Unavailability, dateStr: string) => {
        ...
        return (
            <div
                ...
                } — ${u.type === "Atividade" ? "Atividade/Indisp. Parcial" : u.type}${u.detalhes ? ` (${u.detalhes})` : ""}`}
                className={`flex items-center gap-1.5 h-8 ${
                    unavailColor(u.type)
                } border rounded-lg px-2 cursor-pointer hover:shadow-md transition-all`}
            >
                <span className="material-symbols-outlined text-[14px]">
                    event_busy
                </span>
                <span className="text-[10px] font-black truncate uppercase tracking-tight">
                    {member.war_name || member.name} — {u.type === "Atividade" ? "Atividade/Indisp. Parcial" : u.type}
                </span>
            </div>
        );
    };
    ...
    const renderMissionItem = (m: Mission, dateStr: string) => {
        ...
        return (
            <div
                ...
                title={`${m.nome} — Equipe: ${
                    teamMembers.map((mem) => mem.war_name || mem.name).join(
                        ", ",
                    )
                }`}
            ...
            >
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 opacity-0 group-hover/mission:opacity-100 transition-opacity z-[70] pointer-events-none">
                    <div className="bg-slate-900 dark:bg-slate-800 text-white text-[10px] font-bold rounded-lg px-3 py-2 whitespace-nowrap shadow-xl border border-white/10 relative">
                        <div className="text-cyan-400 mb-1 font-black uppercase text-[9px] tracking-widest border-b border-white/10 pb-1">
                            Equipe da Missão
                        </div>
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 border-[6px] border-transparent border-b-slate-900 dark:border-b-slate-800">
                        </div>
                    </div>
                </div>
            </div>
    ...
    const renderDayContent = (
        dateObj: Date,
        dayTaskItems: { task: Task; isProjected: boolean }[],
        dateStr: string,
    ) => {
                ...
                {displayFilter === "efetivo" &&
                    ...
                    <div className="flex flex-col gap-1 mt-1 pt-1 border-t border-slate-100 dark:border-slate-800">
                        {filteredAllMembers.map((member) => {
                            const isInMission = dayMissions.some((m) =>
                                (m.equipe || []).includes(member.id)
                            );
                            const isInUnavail = dayUnavails.some((u) =>
                                u.member === member.id
                            );
    ...
    return (
        <div className="flex flex-col h-full bg-[#f8fafc] dark:bg-background-dark animate-in fade-in duration-500">
                    ...
                    {currentUser?.sector === "CH" && (
                        <div className="flex bg-slate-200 dark:bg-slate-800 p-1 rounded-xl">
                            ...
                            <button
                                type="button"
                                onClick={() =>
                                    setSelectedSectorFilter("Capacidade")}
                                className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all uppercase tracking-wider ${
                            ...
                            <button
                                type="button"
                                onClick={() =>
                                    setSelectedSectorFilter("Espaço Aéreo")}
                                className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all uppercase tracking-wider ${
                        ...
                        <div className="px-4 md:px-8 py-2 min-w-[150px] md:min-w-[200px] text-center">
                            <span className="text-sm md:text-lg font-bold text-[#0d141b] dark:text-white whitespace-nowrap">
                                {viewMode === "month"
                                    ...
                                    : `Semana de ${
                                        getWeekDays()[0].toLocaleDateString(
                                            "pt-BR",
                                            {
                                                day: "2-digit",
                                                month: "2-digit",
                    ...
                    <div className="min-w-[1000px] h-full flex flex-col">
                        <div className="grid grid-cols-7 border-b border-[#e7edf3] dark:border-slate-800">
                            ...
                            (day) => (
                                <div
                                    key={day}
                                    className="py-5 text-center text-sm font-black text-[#5c85ad] dark:text-slate-400 uppercase tracking-widest"
                                >
                                    {day}
                                </div>
                            ),
                            ...
                            ? (
                                <>
                                    {Array.from({
                                        ...
                                        <div
                                            key={`empty-${i}`}
                                            className="border-r border-b border-[#e7edf3] dark:border-slate-800 bg-slate-50/30 dark:bg-slate-950/20"
                                        />
                                    ))}
                                    ...
                                    {Array.from({
                                        ...
                                        const dayTaskItems = getTasksForDay(
                                            day,
                                        );
                                        const isToday =
                                            today.getDate() === day &&
                                                ...
                                                <div className="flex justify-start mb-2">
                                                    <span
                                                        className={`text-base font-black ${
                                                            isToday
                                                                ? "size-8 flex items-center justify-center bg-blue-600 text-white rounded-full"
            ...
            <div className="px-6 py-4 flex items-center justify-center gap-8 flex-wrap text-sm font-bold text-[#4c739a] dark:text-slate-400">
                ...
                <div className="flex items-center gap-2">
                    <div className="size-3 rounded-full bg-indigo-500 shadow-sm shadow-indigo-200" />
                    <span>Semanal</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="size-3 rounded-full bg-blue-500 shadow-sm shadow-blue-200" />
                    <span>Quinzenal</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="size-3 rounded-full bg-emerald-600 shadow-sm shadow-emerald-200" />
                    <span>Mensal</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="size-3 rounded-full bg-amber-500 shadow-sm shadow-amber-200" />
                    <span>Temporada</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="size-3 rounded-full bg-rose-500 shadow-sm shadow-rose-200" />
                    <span>Pontual</span>
                </div>
                <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[14px]">
            ...
            {showUnavailModal && (
                ...
                >
                    <div
                        className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md mx-4 border border-slate-200 dark:border-slate-700"
                        onClick={(e) => e.stopPropagation()}
                    >
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                >
                                    ...
                                    {filteredMembers
                                        .sort((a, b) =>
                                            ABREV_ORDER.indexOf(a.abrev || "") -
                                            ABREV_ORDER.indexOf(b.abrev || "")
                                        )
                                        .map((m) => (
                                            <option key={m.id} value={m.id}>
                                                {m.abrev} {m.war_name || m.name}
                                            </option>
                                        ))}
                                </select>
                            </div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setUnavailForm({
                                            ...
                                            type: newType,
                                            detalhes: isEnabled
                                                ? unavailForm.detalhes
                                                : "",
                                            atividade: newType === "Atividade"
                                                ? unavailForm.atividade
                                                : "",
                                        });
                                    }}
                                ...
                                >
                                    ...
                                    {UNAVAIL_TYPES.map((t) => (
                                        <option key={t} value={t}>
                                            {t === "Atividade" ? "Atividade/Indisp. Parcial" : t}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder={[
                                            ...
                                            "Outros",
                                            "Atividade",
                                        ].includes(unavailForm.type)
                                        ? "Informações adicionais..."
                                        : "Disponível apenas para Home Office, Dispensa, Atividade e Outros"}
                            ...
                            {/* Radio Buttons de Período - Apenas para Atividade */}
                            {unavailForm.type === "Atividade" && (
                                    ...
                                    <div className="flex gap-3">
                                        {[
                                            ...
                                            >
                                                ...
                                                />
                                                <div className="text-center px-3 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-bold text-slate-500 dark:text-slate-400 transition-all peer-checked:bg-cyan-50 peer-checked:border-cyan-400 peer-checked:text-cyan-700 dark:peer-checked:bg-cyan-900/30 dark:peer-checked:border-cyan-600 dark:peer-checked:text-cyan-400 hover:bg-slate-50 dark:hover:bg-slate-800">
                                                    {opt.label}
                                                </div>
                                            </label>
                                        ))}
                        ...
                        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={saveUnavail}
                                    disabled={!unavailForm.member ||
                                        !unavailForm.type ||
                                        !unavailForm.start_date ||
                                        !unavailForm.end_date ||
                                        (unavailForm.type === "Atividade" &&
                                            !unavailForm.atividade)}
