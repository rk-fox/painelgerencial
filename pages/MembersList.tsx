                                        ...
                                        <>
                                            {filteredMembers.length === 0 ? (
                                                ...
                                                filteredMembers.map((member) => {
                                                    ...
                                                    const displayedStatus = isOnMission ? 'Em Viagem' : (currentUnavail ? currentUnavail.type : member.status);
...
const MemberRow = ({ initials, initialsColor, avatar, name, email, rank, specialty, specialtyColor, entry, status, statusColor, onEdit, onDelete, onToggleStatus, originalStatus, onClick }: any) => {
    const badges: any = {
    ...
    return (
        <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group cursor-pointer" onClick={onClick}>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            ...
            >
                ...
                <div className="relative bg-gradient-to-br from-primary/10 to-primary/5 dark:from-primary/20 dark:to-primary/10 p-8 flex flex-col items-center">
                    ...
                    {/* Name and basic info */}
                    <h3 className="text-xl font-black text-slate-800 dark:text-white mt-4 text-center">Militar</h3>
                    <div className="flex items-center gap-2 mt-1 flex-wrap justify-center">
                        <span className="text-sm font-medium text-slate-500 dark:text-slate-400">{member.rank}</span>
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{member.war_name || member.name}</span>
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                            member.specialty === 'BCT' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                            : member.specialty === 'CTA' ? 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300'
                            : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'
                        }`}>
                            <img
                                src={(member.specialty === 'BCT' || member.specialty === 'CTA') ? 'https://raw.githubusercontent.com/rk-fox/painelgerencial/refs/heads/main/bct-icon-transp.png' : 'https://raw.githubusercontent.com/rk-fox/painelgerencial/refs/heads/main/ais-icon-transp.png'}
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 text-center">
                            {member.sector === 'CP' ? 'Capacidade' : member.sector === 'EA' ? 'Espaço Aéreo' : member.sector || '—'}
                        </h4>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Entrada na Seção</p>
                                <p className="text-sm font-bold text-primary">{formatDate(member.entry_date)}</p>
                            </div>
                            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Última Promoção</p>
                                <p className="text-sm font-bold text-primary">{formatDate(member.last_promotion_date)}</p>
                            </div>
                        </div>
                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center mt-3">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Tempo de Seção</p>
                            <p className="text-sm font-bold text-slate-700 dark:text-slate-300">
                                {member.entry_date ? calculateSectionTime(member.entry_date) : '—'}
                            </p>
                        </div>
                    </div>
                    ...
                    {courses.length > 0 && (
                        <div>
                            <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 text-center">Cursos</h4>
                            <div className="grid grid-cols-2 gap-2">
                                {courses.map((course: string) => (
                                    ...
                                    >
                                        {course === 'ATM043' && 'ATM043 - Planejador de EA'}
                                        {course === 'ATM044' && 'ATM044 - Capacidade de Setor'}
                                        {course === 'ATM045' && 'ATM045 - Capacidade de Pista'}
                                        {course === 'ATM047' && 'ATM047 - Indicadores'}
                                        {course === 'ATM049' && 'ATM049 - Análise Técnica de EA'}
                                        {course === 'AGA001' && 'AGA001 - Introdução à Atividade AGA'}
                                        {course === 'AGA004' && 'AGA004 - Análise Téc. Processos AGA'}
                                        {!['ATM043', 'ATM044', 'ATM045', 'ATM047', 'ATM049', 'AGA001', 'AGA004'].includes(course) && course}
                                    </div>
                                ))}
                            </div>
                        </div>
