...
const ReportsComparative: React.FC = () => {
    ...
    const [viewMode, setViewMode] = useState<'general' | 'category'>('general');
    ...
    // Calculate chart data for General view
    ...
    // Calculate chart data for Category view
    ...
    const activeChartData = viewMode === 'general' ? generalChartData : categoryChartData;
        ...
        <div className="flex flex-col min-h-full bg-[#f8fafc] dark:bg-background-dark animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-4 px-6 py-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-blue-600 dark:text-blue-500 font-bold tracking-widest uppercase mt-0.5">
                        Análise de Desempenho Ano a Ano
                    </p>
                </div>
            </div>
            ...
            <div className="flex-1 p-6 space-y-6 max-w-4xl mx-auto w-full">
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    {/* Category Selector (Visible in Category view) */}
                    {viewMode === 'category' && (
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative min-h-[360px] flex flex-col">
                    <div className="mb-6">
                        <h3 className="text-base font-bold text-slate-800 dark:text-white">
                            {viewMode === 'general' ? 'Comparativo de Atividades Concluídas' : `Comparativo: ${selectedCategory}`}
                        </h3>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                            Análise dos últimos 5 anos ({startYear} - {currentYear})
                        </p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="flex-1 flex flex-col justify-end pt-6">
                            ...
                            <div className="h-64 flex relative border-b border-slate-200 dark:border-slate-800 pb-2">
                                ...
                                {/* Columns Container */}
                                <div className="flex-1 ml-12 h-full flex items-end justify-around relative z-10">
                                    {activeChartData.map((d, idx) => {
                                        const heightPercent = maxVal > 0 ? (d.value / maxVal) * 100 : 0;
                                        return (
                                            <div key={d.year} className="flex flex-col items-center flex-1 h-full justify-end relative group px-2 max-w-[80px]">
                                                {/* Tooltip */}
                                                ...
                                                {/* Bar Column */}
                                                <div
                                                    className={`w-full max-w-[40px] bg-gradient-to-t from-blue-600 to-cyan-400 hover:from-blue-700 hover:to-cyan-500 rounded-t-lg transition-all duration-700 ease-out shadow-lg hover:shadow-blue-200/50 dark:hover:shadow-none min-h-[4px] cursor-pointer`}
                                                    style={{ 
                                                ...
                                                {/* X-Axis Label */}
                                                <span className="absolute -bottom-7 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                                    {d.year}
                                                </span>
                                            </div>
                                        );
