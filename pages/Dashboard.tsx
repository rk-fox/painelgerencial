          ...
          <div className="grid gap-4 min-h-[200px] p-2 bg-slate-50/50 dark:bg-slate-800/10 rounded-2xl border-2 border-dashed border-transparent hover:border-primary/20 transition-colors">
            {myTasks.map((task) => (
              <MyTaskCard
                ...
                onEdit={() => navigate("/app/tasks/new", { state: { editTask: task } })}
              />
            ))}
            {myTasks.length === 0 && (
...
const MyTaskCard = (
  { task, onDragStart, onStart, onSuspend, onComplete, onEdit }: any,
) => {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between border-t border-[#e7edf3] dark:border-slate-800 pt-5">
          ...
          <button
            onClick={onEdit}
            className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-500 hover:text-primary transition-all active:scale-95"
            title="Editar Tarefa"
          >
